...
local function getNearestPlayer()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            local distance = (playerCharacter.HumanoidRootPart.Position - player.Character.HumanoidRootPart.Position)
            .Magnitude
...
local function onPartOwnerAdded(descendant, primaryPart)
    if descendant.Name == "PartOwner" and descendant.Value ~= localPlayer.Name then
        ...
        U.GetDescendant(U.FindFirstAncestorOfType(primaryPart, "Model"), "Highlight", "Highlight")
        if highlight then
...
local function anchorGrab()
    while true do
        pcall(function()
            ...
            local primaryPart = weldConstraint.Part1.Name == "SoundPart" and weldConstraint.Part1 or
            weldConstraint.Part1.Parent.SoundPart or weldConstraint.Part1.Parent.PrimaryPart or weldConstraint.Part1
...
local function compileGroup()
    if #anchoredParts == 0 then
        OrionLib:MakeNotification({ Name = "Error", Content = "No anchored parts found", Image =
        "rbxassetid://4483345998", Time = 5 })
    else
        OrionLib:MakeNotification({ Name = "Success", Content = "Compiled " .. #anchoredParts .. " Toys together", Image =
        "rbxassetid://4483345998", Time = 5 })
    end
                ...
                for _, partModel in pairs(anchoredParts) do
                    coroutine.wrap(function()
                        if partModel then
                            ...
                            if distance <= 30 then
                                local highlight = partModel:FindFirstChild("Highlight") or
                                partModel.Parent:FindFirstChild("Highlight")
...
if localVersion ~= version then
    OrionLib:MakeNotification({ Name = "Script version mismatch!", Content =
    "You seem to have an older version of Ethereon. Original Loadstring has been copied", Image =
    "rbxassetid://4483345998", Time = 8 })
...
local Window = OrionLib:MakeWindow({
    Name = "Ethereon (Owner) - Latest: " .. version,
    HidePremium = false,
    SaveConfig = false,
    ConfigFolder = "Ethereon (Owner)",
    IntroEnabled = true,
...
ObjectGrabTab:AddParagraph("Anchor grab information",
    "If someone grabs your anchored parts, they will fall and you will need to position them again!")
...
ObjectGrabTab:AddParagraph("Compile?",
    "(New) This option allows you to compile all the anchored parts into one. To control this 'Build', you need to move the header part. The first part you grabbed will be the header and will be highlighted green")
        ...
        if enabled then
            ...
            antiKickCoroutine = RunService.Heartbeat:Connect(function()
                ...
                if character and character:FindFirstChild("HumanoidRootPart") and character:FindFirstChild("HumanoidRootPart"):FindFirstChild("FirePlayerPart") then
                    local partOwner = character:FindFirstChild("HumanoidRootPart"):FindFirstChild("FirePlayerPart")
                    :FindFirstChild("PartOwner")
            ...
            blobmanCoroutine = coroutine.create(function()
                ...
                if not foundBlobman then
                    ...
                    OrionLib:MakeNotification({
                        Name = "Error",
                        Content =
                        "You must be mounted upon a blobman to begin this process. Please mount one and toggle this again!",
                        Image = "rbxassetid://4483345998",
                        Time = 5
                    })
                    blobman1:Set(false)
                            ...
                            for _, player in pairs(Players:GetPlayers()) do
                                coroutine.wrap(function()
                                    if player ~= localPlayer and player.Character then
                                        ...
                                        if playerTorso then
                                            local distance = (playerTorso.Position - humanoidRootPart.Position)
                                            .Magnitude
                                            if distance <= auraRadius then
                                                SetNetworkOwner:FireServer(playerTorso,
                                                    playerCharacter.HumanoidRootPart.FirePlayerPart.CFrame)
                                                task.wait(0.1)
                                                local velocity = playerTorso:FindFirstChild("l") or
                                                Instance.new("BodyVelocity", playerTorso)
                                                velocity.Name = "l"
                                                velocity.Velocity = Vector3.new(0, 50, 0)
                                                velocity.MaxForce = Vector3.new(0, math.huge, 0)
                                                Debris:AddItem(velocity, 100)
                                ...
                                if player ~= localPlayer and player.Character then
                                    ...
                                    if playerTorso then
                                        ...
                                        if distance <= auraRadius then
                                            ...
                                            task.wait(0.1)
                                            local force = playerTorso:FindFirstChild("GravityForce") or
                                    ...
                                    if player ~= localPlayer and player.Character then
                                        ...
                                        if playerTorso then
                                            ...
                                            if distance <= auraRadius then
                                                ...
                                                playerCharacter:WaitForChild("HumanoidRootPart").FirePlayerPart,
                                                    playerCharacter.HumanoidRootPart.FirePlayerPart.CFrame)
                                                if not platforms[player] then
                                                    local platform = playerCharacter:FindFirstChild("FloatingPlatform") or
                                                    Instance.new("Part")
...
FunTab:AddButton({
    ...
    Callback = function()
                    ...
                    if bodyPosition and bodyGyro then
                        ...
                        if followMode then
                            ...
                            if nearestPlayer and nearestPlayer.Character and nearestPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                Vector3.new(math.cos(angle) * circleRadius, 0, math.sin(angle) * circleRadius)
        ...
        numDecoys .. " units. Manually click each unit if they don't move", Image = "rbxassetid://4483345998", Time = 5 })
    end
})
...
KeybindSection:AddBind({
    ...
    Callback = function()
        ...
        if not ownedToys["Campfire"] then
            OrionLib:MakeNotification({ Name = "Missing toy", Content = "You do not own the Campfire toy. ", Image =
            "rbxassetid://4483345998", Time = 3 })
            return
        end
        if target and target:IsA("BasePart") then
            local character = target.Parent
...
KeybindSection2:AddBind({
    ...
    Callback = function()
        if not ownedToys["BombMissile"] then
            OrionLib:MakeNotification({ Name = "Missing toy", Content = "You do not own the BombMissile toy. ", Image =
            "rbxassetid://4483345998", Time = 3 })
            return
        end
        local connection
...
KeybindSection2:AddBind({
    ...
    Callback = function()
        if not ownedToys["FireworkMissile"] then
            OrionLib:MakeNotification({ Name = "Missing toy", Content = "You do not own the FireworkMissile toy. ", Image =
            "rbxassetid://4483345998", Time = 3 })
            return
        end
        local connection
...
KeybindSection2:AddBind({
    ...
    Callback = function()
        if #bombList == 0 then
            OrionLib:MakeNotification({ Name = "No bombs", Content = "There are no cached bombs to explode", Image =
            "rbxassetid://4483345998", Time = 2 })
            return
        end
    ...
    Callback = function(enabled)
        if enabled then
            ...
            lightorbitcon = RunService.Heartbeat:Connect(function()
                ...
                lightbitpos = U.GetSurroundingVectors(localPlayer.Character.HumanoidRootPart.Position, usingradius,
                    #lightbitparts, lightbitoffset)
                ...
                for i, v in ipairs(lightbitpos) do
                    ...
                    local lookAtCFrame = CFrame.lookAt(bodyPositions[i].Position,
...
OrionLib:MakeNotification({ Name = "Welcome", Content = "Welcome to Ethereon, Owner ", Image = "rbxassetid://4483345998", Time = 5 })
OrionLib:Init()
        ...
        elseif Config.enabled and (Config.spyOnMyself == true or p ~= player) then
            ...
            if hidden and Config.enabled then
                if Config.public then
                    saymsg:FireServer((Config.publicItalics and "/me " or '') .. "{SPY} [" .. p.Name .. "]: " .. msg,
                        "All")
                else
...
function showBadge(message)
    ...
    StarterGui:SetCore("SendNotification", {
        Title = "Ethereon", -- Title of the badge
        Text = message,     -- Message to display
        Duration = 10,      -- Duration in seconds
    })
end

showBadge("Script Loaded, Enjoy")
